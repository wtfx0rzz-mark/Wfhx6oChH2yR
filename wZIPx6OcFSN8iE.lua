...
-- <<EXT BUS INSERT #1 START>>
-- Minimal Extension Bus (non-invasive). Lets us bolt on features via loadstring / strings.
local __canLoadString = (typeof(loadstring) == "function")
local __EXT_LOADED = false

-- Public API for extensions (read-only service handles + live state accessor).
getgenv().MarkAPI = {
    services = {
        Players = Players,
...
-- Inline extensions you can define as strings. Fill below (Step 2).
local INLINE_EXTENSIONS = {}     -- [name] = [==[...code...]==]
local EXTERNAL_EXTENSIONS = {}   -- { "https://example.com/ext.lua", ... } (optional)

-- Loader (idempotent)
local function __LOAD_EXTENSIONS()
    if __EXT_LOADED then return end
    __EXT_LOADED = true

    if __canLoadString then
        -- Load inline first (deterministic order by key)
        local names = {}
        for k in pairs(INLINE_EXTENSIONS) do table.insert(names, k) end
        table.sort(names)
        for _,name in ipairs(names) do
            local src = INLINE_EXTENSIONS[name]
        ...
        -- Load external URLs (optional)
        for _,url in ipairs(EXTERNAL_EXTENSIONS) do
            if type(url) == "string" and #url > 4 then
...
-- <<EXT BUS INSERT #2 START: register inline extension(s)>>
-- Highlight trees in range (yellow) and blink red on new hits. Polls HitRegisters; no edits to chop logic.
INLINE_EXTENSIONS["tree_hit_highlighter.lua"] = [==[
...
local BLINK_TIME        = 0.20
local RECHECK_INTERVAL  = 0.20

local tracked = {}   -- [Model] = {hl=Highlight, last=hitCount, blinkUntil=t or nil}
local lastScanT = 0
...
local function countAllHitsFor(treeModel)
    local hr = treeModel:FindFirstChild("HitRegisters")
    if not hr then return 0 end
    local cnt = 0
    for k,_ in pairs(hr:GetAttributes()) do
        local n, uid = string.match(k, "^(%d+)_([%d]+)$")
        if n and uid then cnt += 1 end
    end
    return cnt
end
...
local function tickVisuals(dt)
        ...
        for _,rec in ipairs(smallTrees()) do
            ...
            if d <= RANGE_STUDS then
                ...
                if not tracked[rec.model] then
                    ...
                    tracked[rec.model] = {
                        hl = hl,
                        last = countAllHitsFor(rec.model),
                        blinkUntil = nil
                    }
                end
            end
        end
        -- cleanup out-of-range or destroyed
        for m,info in pairs(tracked) do
    ...
    -- Detect new hits and blink
    for m,info in pairs(tracked) do
        local hits = countAllHitsFor(m)
        if hits > info.last then
            info.last = hits
            info.blinkUntil = now + BLINK_TIME
            if info.hl then
                info.hl.OutlineColor, info.hl.FillColor = RED, RED
                info.hl.OutlineTransparency, info.hl.FillTransparency = 0, 0.8
            end
        end
        if info.blinkUntil and now >= info.blinkUntil then
            info.blinkUntil = nil
            if info.hl then
                info.hl.OutlineColor, info.hl.FillColor = YELLOW, YELLOW
                info.hl.OutlineTransparency, info.hl.FillTransparency = 0, 0.9
            end
        end
    end
end
...
con = S.RunService.Heartbeat:Connect(function(dt)
    -- stop if player left or script destroyed
    if not api.state().player then
...
-- <<EXT BUS INSERT #2 END>>

-- <<EXT BUS INSERT #3: run the loader once>>
__LOAD_EXTENSIONS()
